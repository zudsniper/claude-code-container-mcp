...
var SERVER_VERSION = "1.10.6";
var debugMode = process.env.MCP_CLAUDE_DEBUG === "true";
var isFirstToolUse = true;
var serverStartupTime = (/* @__PURE__ */ new Date()).toISOString();
...
__name(findClaudeCli, "findClaudeCli");
...
var ClaudeCodeServer = class {
  static {
    __name(this, "ClaudeCodeServer");
  }
  server;
  claudeCliPath;
  // This now holds either a full path or just 'claude'
  packageVersion;
  // Add packageVersion property
  constructor() {
...
8. Claude can do much more, just ask it!
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (args, call) => {
      ...
      let prompt;
      if (toolArguments && typeof toolArguments === "object" && "prompt" in toolArguments && typeof toolArguments.prompt === "string") {
...
Stdout: ${error.stdout}`;
        ...
        if (error.signal === "SIGTERM" || error.message && error.message.includes("ETIMEDOUT") || error.code === "ETIMEDOUT") {
          throw new McpError(ErrorCode.InternalError, `Claude CLI command timed out after ${executionTimeoutMs / 1e3}s. Details: ${errorMessage}`);
        }
